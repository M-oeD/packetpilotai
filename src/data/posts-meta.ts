...
export const POSTS_META: Record<string, {
  ...
  'network-incident-checklist': {
    ...
    faq: [
      {
        q: 'What should I do first when the network goes down?',
        a: 'Before running any command, write down the exact complaint: who is affected, what they were doing, and when it started. Then confirm network vs. workstation — ping the default gateway from an affected machine. If the gateway responds, work up the stack (DNS, WAN, application); if it fails, work down (cable, switch port, VLAN).',
      },
      {
        q: 'How do I quickly scope a network outage?',
        a: 'Test from a second device on the same VLAN and one on a different VLAN. One device affected means a workstation problem, one VLAN means a switch or VLAN configuration issue, and everything affected points to the core, firewall, or WAN. Scoping takes 30 seconds and cuts the search space in half.',
      },
      {
        q: 'How do I tell if an outage is my ISP or my own equipment?',
        a: 'Run a traceroute from inside the network. If it dies at or immediately after your WAN edge, the problem is upstream. Before opening the ISP ticket, capture the traceroute output, timestamps, and affected scope — a ticket with evidence gets escalated; "internet is down" gets queued.',
      },
    ],
    related: ['diagnose-packet-loss', 'diagnose-dns-failures-fast', 'troubleshoot-slow-network-performance'],
  },
  ...
  'why-claude-gives-wrong-network-configs': {
    ...
    faq: [
      {
        q: 'Why does Claude give me wrong or generic network configs?',
        a: 'Because the prompt lacks operational context. An LLM given a vague request will hallucinate command syntax, mix vendor-specific commands, and substitute placeholder interface names. The fix is not a different AI — it is providing the same information you would give a senior engineer on day one: platform and OS version, interface names and roles, what already exists, and the goal in plain English.',
      },
      {
        q: 'What is a context scaffold for AI prompts?',
        a: 'A short reusable block describing your environment — vendor, platform, OS version, interface names and what they connect to, addressing scheme, and constraints — that you paste at the top of every config request. It takes about two minutes to write once, and it turns generic placeholder output into CLI-ready configs with your real interface names.',
      },
      {
        q: 'How do I stop AI from hallucinating CLI commands?',
        a: 'Pin the platform and version in the prompt (for example "Cisco IOS-XE 17.9"), give real interface names, state the goal in plain English, and ask the model to flag any command it is not certain exists on that version. Most hallucinated syntax comes from the model guessing across vendors and versions because the prompt never said which one you run.',
      },
    ],
    related: ['ai-generate-network-configs', 'setting-up-claude-for-real-work', 'claude-prompt-pack-network-admins'],
  },
};
